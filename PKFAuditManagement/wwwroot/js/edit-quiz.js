...
function updateParticipantsCount() {
    ...
    if (selectedParticipantsInput) {
        // Get the value from the hidden input field
        const selectedParticipants = selectedParticipantsInput.value || '';

        // Split the value by delimiter and filter out empty entries
        const participantEmails = selectedParticipants.split(';').filter(email => email.trim() !== '');

        // Count the number of non-empty entries
        const count = participantEmails.length;

        // Update the participants count display
        participantsCountSpan.textContent = `${count} participants selected`;
